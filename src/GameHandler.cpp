...
void GameHandler::UpdateThrust(int y, int delta) {
  ...
  p->setThrust(thrust);
  p->setVelocity(thrust * cos(p->getDir() * PI / 180), thrust * sin(p->getDir() * PI/180));
}
...
void GameHandler::CollisionsCheck(int delta) {
  ...
  Link* current = getLinkedList(vectors, p->getX());
  ...
  if (current->data->getType() == 1 && current->next->data->getType() == 1) {
